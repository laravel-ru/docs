...
Ограничители скорости определяются с помощью метода `for` фасада `RateLimiter`. Метод `for` принимает имя ограничителя скорости и замыкание, которое возвращает конфигурацию ограничения, которая должна применяться к маршрутам, которым назначен этот ограничитель скорости:
...
Используя новый метод `catch`, теперь Вы можете обеспечить замыкание, которое должно выполняться, если замыкание в очереди не удалось успешно завершить после исчерпания всех настроенных попыток повтора Вашей очереди:
