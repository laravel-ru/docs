...
Этот каталог не существует по умолчанию, но будет создан для вас, если вы выполните Artisan-команду `make:notification`. Каталог `Notifications` содержит все «транзакционные» [уведомления](/docs/{{version}}/notifications), которые отправляются вашим приложением, например простые уведомления о событиях, которые происходят в вашем приложении. Функция уведомлений Laravel абстрагируется от отправки уведомлений через различные драйверы, такие как электронная почта, Slack, SMS или хранящиеся в базе данных.
...
Каталог `Providers` содержит всех [сервис провайдеров](/docs/{{version}}/providers) для Вашего приложения. Сервис провайдеры загружают Ваше приложение, привязывая службы к контейнеру служб, регистрируя события или выполняя любые другие задачи для подготовки Вашего приложения к входящим запросам.
