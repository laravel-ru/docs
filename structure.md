...
Файл `web.php` содержит маршруты, которые `RouteServiceProvider` помещает в мидлвар обеспечения `web`, которая обеспечивает состояние сеанса, защиту CSRF и шифрование файлов cookie. Если Ваше приложение не предлагает RESTful API, без сохранения состояния, все Ваши маршруты, скорее всего, будут определены в файле `web.php`.

Файл `api.php` содержит маршруты, которые `RouteServiceProvider` помещает в мидлвар обеспечения `api`, которая обеспечивает ограничение скорости. Эти маршруты предназначены для без сохранения состояния, поэтому запросы, поступающие в приложение по этим маршрутам, предназначены для аутентификации с помощью токенов и не будут иметь доступа к состоянию сеанса.
