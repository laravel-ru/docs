...
Laravel стремится обеспечить потрясающий опыт разработчика, предоставляя при этом мощные функции, такие как тщательное внедрение зависимостей, выразительный уровень абстракции базы данных, очереди и запланированные задания, модульное и интеграционное тестирование и многое другое.
...
Laravel может служить полноценным фреймворком. Под фреймворком "полный стек" мы подразумеваем, что Вы собираетесь использовать Laravel для маршрутизации запросов к Вашему приложению и рендеринга интерфейса через [шаблонизатор Blade](/docs/{{version}}/blade) или используя гибрид одностраничного приложения, такие технологии, как [Inertia.js](https://inertiajs.com). Это наиболее распространенный способ использования фреймворка Laravel.
