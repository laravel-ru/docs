...
Как уже упоминалось, Вы также можете указать класс `Illuminate\Http\Request` при замыкании маршрута. Сервисный контейнер автоматически вставит входящий запрос в замыкание при его выполнении:
...
Метод `whenHas` выполнит данное замыкание, если в запросе присутствует значение:
